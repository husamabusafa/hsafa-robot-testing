...
class UnifiedBridge:
    ...
    async def _handle_query_ksu_knowledge(
        ...
        try:
            ...
            if result["found"]:
                # Build a concise answer from the results
            ...
            # Speak the answer via Gemini
            await self._handle_say_this({"text": answer, "urgency": "normal"}, ctx)
            return {"ok": True, "found": result["found"], "sources": result.get("sources", [])}
        except Exception as exc:
            ...
            if result["found"]:
                ...
                for f in items:
                    ...
                    if f.get("profile_url"):
                        line += f" | {f['profile_url']}"
                    lines.append(line)
                answer = (
                    f"وجدت {result['count']} نتيجة في قاعدة بيانات هيئة التدريس:\n"
                    + "\n".join(lines)
                )
            else:
                answer = result.get("message", "لم أجد نتائج.")
            await self._handle_say_this({"text": answer, "urgency": "normal"}, ctx)
            return {"ok": True, "found": result["found"], "count": result.get("count", 0)}
        except Exception as exc:
